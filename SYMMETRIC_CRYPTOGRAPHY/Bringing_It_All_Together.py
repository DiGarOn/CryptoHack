...
def decrypt(key, ciphertext):
    ...
    ciph_mathix = bytes2matrix(ciphertext)

    # Initial add round key step
    ciph_mathix = add_round_key(ciph_mathix, round_keys[-1])

    for i in range(N_ROUNDS - 1, 0, -1):
        inv_shift_rows(ciph_mathix)
        ciph_mathix = sub_bytes(ciph_mathix, inv_s_box)
        ciph_mathix = add_round_key(ciph_mathix, round_keys[i])
        inv_mix_columns(ciph_mathix)


    # Run final round (skips the InvMixColumns step)
    inv_shift_rows(ciph_mathix)
    ciph_mathix = sub_bytes(ciph_mathix, inv_s_box)
    ciph_mathix = add_round_key(ciph_mathix, round_keys[0])
    # Convert state matrix to plaintext
    plaintext = matrix2bytes(ciph_mathix)
